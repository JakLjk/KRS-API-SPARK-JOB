...
def krs_api_job():
    ...
    df_registry = load_delta(spark, ENV_DELTA_SILVER_REGISTRY, sort_within_partition_by=["company_krs", "entry_number"])
    df_hist_names = load_delta(spark, ENV_DELTA_SILVER_HISTORY_NAMES)
    df_hist_identifiers = load_delta(spark, ENV_DELTA_SILVER_HISTORY_IDENTIFIERS)
    df_hist_legal_form = load_delta(spark, ENV_DELTA_SILVER_HISTORY_LEGAL_FORM)
    df_hist_has_opp_status = load_delta(spark, ENV_DELTA_SILVER_HISTORY_HAS_OPP_STATUS)
    df_hist_does_conduct_business_with_other_entities = load_delta(spark, ENV_DELTA_SILVER_HISTORY_BUSINESS_WITH_OTHERS)

    df_hist_address = load_delta(spark, ENV_DELTA_SILVER_HISTORY_ADDRESS)
    df_hist_headquarters = load_delta(spark, ENV_DELTA_SILVER_HISTORY_HEADQUARTERS)
    df_hist_webpage = load_delta(spark, ENV_DELTA_SILVER_HISTORY_WEBPAGE)
    df_hist_email = load_delta(spark, ENV_DELTA_SILVER_HISTORY_EMAIL)
    df_hist_electronic_delivery_addres_base = load_delta(spark, ENV_DELTA_SILVER_HISTORY_ELECTRONIC_BAE_DELIVERY)
    df_hist_main_headquarters_address = load_delta(spark, ENV_DELTA_SILVER_HISTORY_MAIN_HQ_ADDRESS)

    df_hist_time_for_which_the_entity_was_created = load_delta(spark, ENV_DELTA_SILVER_HISTORY_TIME_ENTITY_CREATED)
    df_hist_conclusion_or_amendment_of_the_statue_agreement = load_delta(spark, ENV_DELTA_SILVER_HISTORY_STATUTE_CHANGE)

    df_hist_share_capital_amount = load_delta(spark, ENV_DELTA_SILVER_HISTORY_SHARE_CAPITAL_AMOUNT)
    df_hist_total_number_of_shares_units = load_delta(spark, ENV_DELTA_SILVER_HISTORY_TOTAL_SHARES_UNITS)
    df_hist_single_share_value = load_delta(spark, ENV_DELTA_SILVER_HISTORY_SINGLE_SHARE_VALUE)
    df_hist_paid_in_capital_part = load_delta(spark, ENV_DELTA_SILVER_HISTORY_PAID_IN_CAPITAL_PART)

    df_hist_share_issues_series_name = load_delta(spark, ENV_DELTA_SILVER_HISTORY_SHARE_ISSUES_SERIES_NAME)
    df_hist_share_issues_shares_in_series = load_delta(spark, ENV_DELTA_SILVER_HISTORY_SHARE_ISSUES_SHARES_IN_SERIES)
    df_hist_share_issues_preference_info = load_delta(spark, ENV_DELTA_SILVER_HISTORY_SHARE_ISSUES_PREFERENCE_INFO)

    spark.sparkContext.setCheckpointDir("hdfs://192.168.0.12:8020/user/thinkpad/delta/checkpoints")

    # TODO make full history table with row that is is_current.

    print("Partitions:", df_registry.rdd.getNumPartitions())

    base_df = df_registry.select(
        F.col("source_id"),
        F.col("company_krs"),
        F.col("entry_number"),
        F.col("entry_date"),
    ...
    base_df = join_asof(base_df, df_hist_names, ["company_name"])
    base_df = join_asof(base_df, df_hist_identifiers, ["nip_number", "regon_number"])
    base_df = join_asof(base_df, df_hist_legal_form, ["legal_form"])
    base_df = join_asof(base_df, df_hist_has_opp_status, ["has_opp_status"])
    base_df = join_asof(
        base_df,
        df_hist_does_conduct_business_with_other_entities, 
    ...
    base_df = join_asof(
        ...
        "headquarters")
    base_df = join_asof(base_df, df_hist_webpage, ["webpage"])
    base_df = join_asof(base_df, df_hist_email, ["email"])
    base_df = join_asof(
        base_df, 
        df_hist_electronic_delivery_addres_base, 
        ["electronic_bae_delivery"])
    base_df = join_asof(
        base_df, 
        df_hist_main_headquarters_address, 
        [
            "country",
            "street",
            "house_number",
    ...
    base_df = join_asof(
        base_df,
        df_hist_time_for_which_the_entity_was_created, 
        ["time_for_which_entity_was_created"])
    base_df = join_asof(
        base_df, 
        df_hist_conclusion_or_amendment_of_the_statue_agreement, 
        ["change_or_amendment_of_statue_agreement"])
    base_df = join_asof(
        base_df, 
        df_hist_share_capital_amount, 
        ["currency", "share_capital_amount"],
        "shareCapitalAmount")
    base_df = join_asof(
        base_df, 
        df_hist_total_number_of_shares_units, 
        ["total_number_of_shares_units"])
    base_df = join_asof(
        base_df, 
        df_hist_single_share_value, 
    ...
    base_df = join_asof(
        base_df, 
        df_hist_paid_in_capital_part, 
        ["currency", "paid_in_capital_amount"],
        "paidInCapitalPart")
    base_df = join_asof(
        base_df, 
        df_hist_share_issues_series_name, 
        ["series_name"],
        "sharesIssued")
    base_df = join_asof(
        base_df, 
        df_hist_share_issues_shares_in_series, 
        ["shares_in_series"],
        "sharesInSeries")
    base_df = join_asof(
        base_df, 
        df_hist_share_issues_preference_info, 
        ["preference_info"],
        "sharesIssued")


    base_df.write.format("delta").mode("overwrite").save(ENV_DELTA_GOLD_STAGING)
